...
public class FoundationVM {
	...
	private List<String> templates = Arrays.asList("Banded", "Blog", "Feed", "Grid", "Orbit Home", "Banner Home",
			"Sidebar", "Contact", "Marketing", "Realty", "So Boxy", "Store", "Workspace", "Marketing 2", "Product",
			"Portfolio");
		...
		if (source == null) {
			...
			try {
				...
				source = FileUtils.readFileToString(
						new File(Sessions.getCurrent().getWebApp().getRealPath(link)),
						"UTF-8");
